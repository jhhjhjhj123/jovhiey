...
3. **Push (Delivery)** - Use `atlas migrate push`, or set up a CI pipeline to push the latest migrations state to the
   [Atlas Schema Registry](/cloud/features/registry). Alternatively, you can package the migrations directory into
   a custom Docker image and push it to an artifactory.
...
directly from there without needing to have them locally. For example, `atlas migrate apply --dir "atlas://app"` will
apply the pending migrations of the `app` project based on the most recent pushed state. To see it in action, run the following:
...
By using the `dry-run` option, Atlas prints the migration files and their SQL statements that are pending to be applied
without executing them. However, if the migration plan contains [pre-migration checks](/versioned/checks), Atlas executes
...
```shell
atlas schema clean -u "redshift://user:pass@redshift-cluster:5439/database?search_path=public&sslmode=disable"
atlas migrate apply -u "redshift://user:pass@redshift-cluster:5439/database?search_path=public&sslmode=disable"
```
